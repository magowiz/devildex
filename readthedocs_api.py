...
def download_readthedocs_prebuilt_robust(rtd_url, preferred_versions=['stable', 'latest'], download_format='htmlzip'):
    """
    ...
    api_list_versions_url = f"https://readthedocs.org/api/v3/projects/{project_slug}/versions/"
    ...
    try:
        ...
        print(f"API lista versioni chiamata con successo. Trovate {versions_list_data.get('count', 'N/A')} versioni.")

    except requests.exceptions.RequestException as e:
        print(f"Errore chiamando API lista versioni ({api_list_versions_url}): {e}")
        print("Impossibile ottenere l'elenco delle versioni. Non posso procedere a scegliere la versione.")
        return None
    ...
    if not available_versions:
        print("Errore: L'API non ha restituito nessuna versione disponibile per questo progetto.")
        return None

    print("Versioni disponibili (slug, attivo, costruito):")
    for version in available_versions:
         print(f"- {version.get('slug')}: Active={version.get('active')}, Built={version.get('built')}")


    # Cerca tra le versioni preferite se sono attive e costruite
    for preferred in preferred_versions:
        for version in available_versions:
            if version.get('slug') == preferred and version.get('active') and version.get('built'):
    ...
    if not chosen_version_slug:
         ...
         for version in available_versions:
             if version.get('active') and version.get('built'):
                 ...
                 print(f"Scelta prima versione attiva e costruita: '{chosen_version_slug}'")
                 break
    ...
    api_version_detail_url = f"https://readthedocs.org/api/v3/versions/{chosen_version_slug}/"
    ...
    try:
        ...
        response = requests.get(api_version_detail_url, params={'project__slug': project_slug})
        response.raise_for_status() # Genera eccezione per errori HTTP (come 404)

        version_detail_data = response.json()
        print(f"API dettagli versione chiamata con successo per '{chosen_version_slug}'.")

    except requests.exceptions.RequestException as e:
        print(f"Errore chiamando API dettagli versione ({api_version_detail_url}?project__slug={project_slug}): {e}")
        print("Impossibile ottenere i dettagli della versione. Non posso ottenere i link di download.")
        return None
    except Exception as e:
        print(f"Si è verificato un errore inatteso ottenendo i dettagli della versione: {e}")
        return None

    # --- Passo 5: Estrarre l'URL di download ---
    download_urls = version_detail_data.get('downloads')
    if not download_urls:
        print(f"Errore: Campo 'downloads' non trovato nei dettagli per la versione '{chosen_version_slug}'.")
        print("Assicurati che i formati offline siano abilitati per questa versione.")
        return None

    file_url = download_urls.get(download_format)
    if not file_url:
         print(f"Errore: Formato '{download_format}' non disponibile per la versione '{chosen_version_slug}'.")
    ...
    if not file_url.lower().endswith(('.zip', '.pdf', '.epub')):
         print(f"Avviso: L'URL trovato '{file_url}' potrebbe non essere un link diretto al file scaricabile (estensione non riconosciuta). Procedo comunque...")
...
downloaded_file = download_readthedocs_prebuilt_robust('https://requests.readthedocs.io/')
if downloaded_file:
