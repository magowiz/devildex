...
try:
    if not config.read(config_file, encoding='utf-8'):
        print(f"Avviso: File di configurazione '{config_file}' non trovato. Uso i valori di default.")
        ...
        custom_banner = config.get('customization', 'banner_text', fallback=default_banner)
        # custom_css = config.get('customization', 'custom_css_file', fallback=None) # Esempio
        print(f"Configurazione letta da '{config_file}': Tema='{custom_theme}', Banner='{custom_banner}'")

except configparser.Error as e:
    print(f"Errore nella lettura del file di configurazione '{config_file}': {e}. Uso i valori di default.")
    custom_theme = default_theme
    custom_banner = default_banner
    # custom_css = None
# --- Fine lettura configurazione ---
def apply_devildex_customizations(isolated_source_path, theme_name, banner_text): # <-- Aggiunti parametri
    """
    ...
    """
    print(f"\nApplying DevilDex customizations (Theme: {theme_name}, Banner: '{banner_text}')...") # Log aggiornato
    ...
    custom_css_path = os.path.join(static_dir, 'custom.css') # Nome fisso per semplicità

    # --- 1. Assicura che le directory esistano ---
    try:
        os.makedirs(templates_dir, exist_ok=True)
        os.makedirs(static_dir, exist_ok=True)
        print(f"  - Assicurate directory: _templates, _static")
    except OSError as e:
        print(f"  - Errore creazione directory _templates/_static: {e}")

    # --- 2. Crea/Sovrascrivi override del template layout.html ---
    #    Inserisce il banner nel footer (verrà spostato da JS)
    layout_content = f"""\
    {{% extends "!layout.html" %}}
    {{% block relbar1 %}}
    ...
    try:
        ...
        print(f"  - Creato/Aggiornato override template: {os.path.basename(layout_template_path)}")
    except Exception as e:
        print(f"  - Errore scrittura override template {layout_template_path}: {e}")

    # --- 3. Crea/Sovrascrivi custom.css (per stile opzionale) ---
    css_content = """\
    ...
    try:
        with open(custom_css_path, 'w', encoding='utf-8') as f:
            f.write(css_content)
        print(f"  - Creato/Aggiornato CSS personalizzato: {os.path.basename(custom_css_path)}")
    except Exception as e:
        print(f"  - Errore scrittura CSS personalizzato {custom_css_path}: {e}")

    # --- 4. Modifica conf.py per usare tema e includere personalizzazioni ---
    try:
        ...
        # a) Imposta/Forza html_theme usando theme_name passato
        theme_pattern = re.compile(r"^\s*html_theme\s*=\s*['\"].*['\"]", re.MULTILINE)
        target_theme_line = f"html_theme = '{theme_name}'" # <-- Usa theme_name
        if theme_pattern.search(new_conf_content):
            ...
            if old_theme_line != target_theme_line:
                new_conf_content = theme_pattern.sub(target_theme_line, new_conf_content, count=1)
        ...
        # b) Assicura templates_path (prepend)
        templates_pattern = re.compile(r"^\s*templates_path\s*=\s*(\[.*?\])", re.MULTILINE | re.DOTALL)
        ...
        if match:
            ...
            if "'_templates'" not in current_list_str:
                # Prepend '_templates'
                try:
                    # Prova a valutare la lista esistente per aggiungerci l'elemento
                    import ast
                    current_list = ast.literal_eval(current_list_str)
                    if isinstance(current_list, list):
                        new_list = ['_templates'] + current_list
                        new_list_str_repr = repr(new_list)
                        new_conf_content = new_conf_content.replace(match.group(0), f"templates_path = {new_list_str_repr}", 1)
                        print(f"  - Prepend '_templates' a templates_path in conf.py")
                        conf_updated = True
                    else:
                        raise ValueError("Non è una lista")
                except (ValueError, SyntaxError):
                    # Fallback se ast.literal_eval fallisce: concatenazione di stringhe (meno robusta)
                    print("  - Avviso: Fallback per templates_path, uso concatenazione stringhe.")
                    new_list_str = "['_templates'] + " + current_list_str
                    new_conf_content = new_conf_content.replace(match.group(0), f"templates_path = {new_list_str}", 1)
                    conf_updated = True

        else:
            new_conf_content += "\ntemplates_path = ['_templates']\n"
            print(f"  - Aggiunto templates_path = ['_templates'] a conf.py")
            conf_updated = True

        # c) Assicura html_static_path (append)
        static_pattern = re.compile(r"^\s*html_static_path\s*=\s*(\[.*?\])", re.MULTILINE | re.DOTALL)
            ...
            if "'_static'" not in current_list_str:
                 try:
                    ...
                    if isinstance(current_list, list):
                        ...
                        new_conf_content = new_conf_content.replace(match.group(0), f"html_static_path = {new_list_str_repr}", 1)
                        print(f"  - Append '_static' a html_static_path in conf.py")
                        conf_updated = True
                    else:
                        raise ValueError("Non è una lista")
                 except (ValueError, SyntaxError):
                    print("  - Avviso: Fallback per html_static_path, uso concatenazione stringhe.")
                    ...
                    if new_list_str and new_list_str != '[': # Se la lista non era vuota
                         ...
                         new_list_str = "['_static']"
                    new_conf_content = new_conf_content.replace(match.group(0), f"html_static_path = {new_list_str}", 1)
                    conf_updated = True
        else:
            new_conf_content += "\nhtml_static_path = ['_static']\n"
            print(f"  - Aggiunto html_static_path = ['_static'] a conf.py")
            conf_updated = True

        # d) Assicura html_css_files (append 'custom.css')
        css_pattern = re.compile(r"^\s*html_css_files\s*=\s*(\[.*?\])", re.MULTILINE | re.DOTALL)
        match = css_pattern.search(new_conf_content)
        css_file_to_add = 'custom.css' # Nome fisso
        if match:
            current_list_str = match.group(1)
            if f"'{css_file_to_add}'" not in current_list_str and f'"{css_file_to_add}"' not in current_list_str:
                 try:
                    import ast
                    current_list = ast.literal_eval(current_list_str)
                    if isinstance(current_list, list):
                        ...
                        new_list_str_repr = repr(current_list)
                        new_conf_content = new_conf_content.replace(match.group(0), f"html_css_files = {new_list_str_repr}", 1)
                        print(f"  - Append '{css_file_to_add}' a html_css_files in conf.py")
                        conf_updated = True
                    else:
                        raise ValueError("Non è una lista")
                 except (ValueError, SyntaxError):
                    print(f"  - Avviso: Fallback per html_css_files, uso concatenazione stringhe.")
                    new_list_str = current_list_str.rstrip()[:-1].strip()
                    if new_list_str and new_list_str != '[':
                         new_list_str += f", '{css_file_to_add}']"
                    else:
                         new_list_str = f"['{css_file_to_add}']"
                    new_conf_content = new_conf_content.replace(match.group(0), f"html_css_files = {new_list_str}", 1)
                    conf_updated = True
        else:
        ...
        if conf_updated:
            ...
            print(f"  - Riscritto conf.py con aggiornamenti.")
        else:
            print(f"  - conf.py non necessitava di modifiche per le personalizzazioni.")
...
def download_readthedocs_source_and_build(rtd_url):
    ...
    if isolated_source_path:
        ...
        apply_devildex_customizations(isolated_source_path,
