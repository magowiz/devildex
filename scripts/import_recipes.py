...
from devildex.database.db_manager import DatabaseManager
...
class RecipeImporter:
    ...
    def save_project_doc_requirements(
        ...
        with self.db_session() as session:
            ...
            package_info = session.query(PackageInfo).filter_by(package_name=package_name).first()
            if not package_info:
                package_info = PackageInfo(package_name=package_name)
                session.add(package_info)
                session.flush() # Assigns primary key to package_info

            # Find or create ProjectDocRequirements
            doc_req = session.query(ProjectDocRequirements).filter_by(
...
def main():
    ...
    importer = RecipeImporter(db_session=Session)
