...
class AppPaths:
    ...
    def __init__(self, app_name: str = APPLICATION_NAME, app_author: str = APPLICATION_AUTHOR, version: str | None = None):
        """Inizializza i percorsi.

        Args:
            app_name: Il nome dell'applicazione.
            app_author: L'autore/organizzazione dell'applicazione (rilevante per Windows).
            version: Una versione opzionale dell'app, per percorsi versionati.
        """
        self._dirs = platformdirs.PlatformDirs(appname=app_name, appauthor=app_author, version=version)
    ...
    def user_config_dir(self) -> Path:
        ...
        Es. macOS:   ~/Library/Application Support/devildex  (platformdirs spesso usa questa anche per config)
        Es. Windows: C:\\Users\\<Utente>\\AppData\\Roaming\\DevilDexTeam\\devildex\\Config
        """
    ...
    def settings_file_path(self) -> Path:
        """Percorso per il file delle impostazioni dell'applicazione (es. settings.toml o settings.json).
        """
...
if __name__ == "__main__":
    paths = AppPaths()
    print(f"User Data Directory:     {paths.user_data_dir}")
    print(f"User Config Directory:   {paths.user_config_dir}")
    print(f"User Cache Directory:    {paths.user_cache_dir}")
    print(f"User Log Directory:      {paths.user_log_dir}")
    print(f"Docsets Base Directory:  {paths.docsets_base_dir}")
    print(f"Database Path:           {paths.database_path}")
    print(f"Settings File Path:      {paths.settings_file_path}")
