...
def generate_docs_from_folder(input_folder: str, output_folder: str, modules_to_document: list[str] | None = None) -> bool:
    """
    ...
    Returns:
        ...
        False altrimenti (es. cartella non trovata, nessun modulo trovato, importazione fallita per tutti).
    """
    ...
    if modules_to_document is not None:
        print(f"Utilizzo moduli specificati: {modules_to_document} dalla cartella: {input_folder}")
        ...
        for item_name in os.listdir(input_folder):
            ...
            if os.path.isfile(item_path) and item_name.endswith('.py'):
                ...
                if os.path.exists(os.path.join(item_path, '__init__.py')):
                    ...
                    print(f"Trovato pacchetto: {package_name}")
                    names_to_import.append(package_name)

    if not names_to_import:
        print(f"Nessun modulo o pacchetto Python trovato/specificato in '{input_folder}'. Nessuna documentazione generata.")
        return False
    ...
    try:
        for name in names_to_import:
            try:
                module_obj: ModuleType = pdoc.import_module(name, reload=False, skip_errors=True)

                if not hasattr(module_obj, '__file__') and not hasattr(module_obj, '__name__'):
                     print(f"WARNING: L'importazione di '{name}' è fallita (import_module con skip_errors=True ha ritornato un dummy). Saltato.")
                     continue

                wrapped_modules.append(pdoc.Module(module_obj, context=context))
                print(f"Importato e wrappato: {name}")
            except Exception as e:
                 print(f"AVVISO: Errore imprevisto durante l'importazione o wrapping di '{name}': {e.__class__.__name__}: {e}")

        if not wrapped_modules:
            print("Nessun modulo è stato importato correttamente dopo aver tentato l'importazione dei nomi trovati/specificati.")
            return False
        ...
        for root_module_obj in wrapped_modules:
            for current_pdoc_module in recursive_htmls(root_module_obj):
                ...
                try:
                    ...
                    print(f"Saved: {full_output_path} (Module: {current_pdoc_module.qualname}, URL from pdoc: {relative_url_path})")
                except IOError as e:
                    print(f"Error durante il salvataggio di {full_output_path}: {e}")

        print("Generazione documentazione completed.")
        return True
...
def run(url, project_name, version=""):
    ...
    try:
        ...
        result = subprocess.run(get_site_packages_command, check=True, capture_output=True, text=True, encoding="utf-8")
        venv_site_packages = result.stdout.strip()
        subprocess.run([venv_python_interpreter, "-m", "pip", "install", "."],
                       check=True, capture_output=True, text=True, encoding="utf-8",
                       cwd=cloned_repo_path
                       )

        subprocess.run([venv_python_interpreter, "-m", "pip", "install", "pdoc3"],
                       check=True, capture_output=True, text=True, encoding="utf-8"
                       )
        print('qua')
        ...
        try:
            ...
            success = generate_docs_from_folder(cloned_repo_path, tmp_output_dir, modules_to_document=[project_name])

            if success:
                print(f"Generazione documentazione completata con successo per {project_name}")
                if os.path.exists(final_output_dir):
                    shutil.rmtree(final_output_dir)
                shutil.move(tmp_output_dir, final_output_dir)
            else:
                print(f"Generazione documentazione fallita o nessun modulo documentato per {project_name}")

        finally:
            sys.path = original_sys_path_inner
            print(f"Rimosso {venv_site_packages} da sys.path")
        ...
        print("--- TRACEBACK ---")
        traceback.print_exc()
        print(f"--- FINE DETTAGLIO ERROR IMPREVISTO ---")
    finally:
        ...
        print("Pulizia delle cartelle temporanee completed.")
