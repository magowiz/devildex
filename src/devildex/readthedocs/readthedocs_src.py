...
def build_sphinx_docs(
    ...
    if not conf_py_file.exists():
        logger.error(f"Critical Error: conf.py not found in {source_dir_path}.")
        return None

    # Determina il percorso del file requirements.txt per la documentazione
    doc_requirements_file: Path | None = None
    # Lista di candidati per il file requirements.txt
    # (più specifico prima, più generale dopo)
    candidate_req_paths = [
    ...
    # --- INIZIO BLOCCO MODIFICATO/AGGIUNTO per FINAL_OUTPUT_DIR ---
    ...
    try:
        with IsolatedVenvManager(
            ...
            if not install_success:
                logger.error(
                    f"CRITICAL: Installation of project/dependencies (including Sphinx) "
                    f"for {project_slug} FAILED or had critical issues. Aborting Sphinx build."
                )
                return None
