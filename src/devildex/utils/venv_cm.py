...
class IsolatedVenvManager:
    ...
    def _create_venv(self):
        ...
        try:
            ...
            logger.error(f"An unexpected error occurred during venv creation for '{self.project_name}': {e}")
            self._cleanup()
            raise
    ...
    def _upgrade_pip(self):
        ...
        try:
            ...
            subprocess.run(
                ...
                check=True, capture_output=True, text=True, cwd=self.venv_path
            )
            logger.info("Pip upgraded successfully in venv.")
        except subprocess.CalledProcessError as e:
            # Non è critico se l'upgrade di pip fallisce, quindi solo un warning
            logger.warning(f"Failed to upgrade pip in venv '{self.venv_path}': {e.stderr}")

    def _cleanup(self):
        """Removes the temporary virtual environment directory."""
        if self.venv_path and self.venv_path.exists():
            logger.info(f"Cleaning up temporary venv for '{self.project_name}' at: {self.venv_path}")
            try:
                ...
                logger.error(f"Error removing venv '{self.venv_path}': {e}. Manual cleanup might be needed.")
        self.venv_path = None
    ...
    def __enter__(self):
        ...
        if not self.python_executable or not self.pip_executable:
            raise RuntimeError(f"Venv for {self.project_name} was not properly initialized.")
