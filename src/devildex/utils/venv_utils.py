...
def install_project_and_dependencies_in_venv(
    ...
    project_name: str,
    project_root_for_install: Path | None, # Path alla radice del progetto per 'pip install -e .'
    doc_requirements_path: Path | None, # Path completo al file requirements.txt per i docs
    ...
    sphinx_install_cmd = [pip_executable, "install", "--disable-pip-version-check", "--no-python-version-warning",
                          ] + base_packages_to_install
    _, _, ret_code_sphinx = execute_command(
        sphinx_install_cmd,
        f"Install/Verify Sphinx for {project_name}"
    )
    if ret_code_sphinx != 0:
        logger.error(f"CRITICAL: Failed to install/verify Sphinx in venv for '{project_name}'. Sphinx build will fail.")
    ...
    if project_root_for_install and project_root_for_install.exists():
        ...
        if not ((project_root_for_install / "setup.py").exists() or \
                (project_root_for_install / "setup.cfg").exists() or \
                (project_root_for_install / "pyproject.toml").exists()):
            logger.info(f"No setup.py, setup.cfg, or pyproject.toml found in '{project_root_for_install}'. Skipping editable install of project '{project_name}'.")
        else:
            logger.info(f"Attempting to install project '{project_name}' from '{project_root_for_install}' in editable mode into the venv...")
            install_cmd = [pip_executable, "install", "--disable-pip-version-check", "--no-python-version-warning", "-e", "."]
            ...
            if ret_code == 0:
                logger.info(f"Project '{project_name}' installed successfully (or already present) in venv.")
            else:
                logger.error(f"Failed to install project '{project_name}' in venv. Error details in DEBUG log.")
                success = False # Potresti voler interrompere qui
    else:
        logger.info(f"No project root provided for installation, or path does not exist. Skipping editable install of project '{project_name}'.")
    ...
    if doc_requirements_path and doc_requirements_path.exists():
        ...
        if ret_code == 0:
            logger.info(f"Dependencies from '{doc_requirements_path}' installed successfully (or already present) in venv.")
        else:
            logger.error(f"Failed to install dependencies from '{doc_requirements_path}' in venv. Error details in DEBUG log.")
            success = False
    elif doc_requirements_path: # Se il path era fornito ma non esiste
        logger.warning(f"Documentation requirements file not found at '{doc_requirements_path}', skipping.")
    else:
        logger.info(f"No documentation requirements file provided for {project_name}, skipping dependency installation.")
    return success


def execute_command(command: list[str], description: str, cwd: str | Path | None = None, env=None) -> tuple[str, str, int]:
    ...
    try:
        ...
        process = subprocess.run(command, capture_output=True, text=True, check=False, cwd=cwd_str, encoding="utf-8", errors="replace", env=current_env)
        ...
        logger.error(f"Command not found: {command[0]}. Ensure it's in PATH or provide full path.")
