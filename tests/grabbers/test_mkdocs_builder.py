...
def mkdocs_project_setup(tmp_path: Path) -> Path:
    """Sets up a basic MkDocs project in a temporary directory."""
...
def mkdocs_project_with_requirements(mkdocs_project_setup: Path) -> Path:
    """Sets up an MkDocs project with a requirements.txt file."""
...
class TestMkDocsBuilder:
    def test_can_handle_true(self, mkdocs_project_setup: Path, mock_build_context: BuildContext) -> None:
    ...
    def test_can_handle_false(self, tmp_path: Path, mock_build_context: BuildContext) -> None:
    ...
    def test_generate_docset_success(self, mocker: MockerFixture, mkdocs_project_with_requirements: Path, mock_build_context: BuildContext, tmp_path: Path) -> None:
        """Test generate_docset successfully builds documentation."""
        builder = MkDocsBuilder()

        # Mock internal helper methods that are now part of the builder
        mock_find_config = mocker.patch("devildex.grabbers.mkdocs_builder._find_mkdocs_config_file", return_value=mkdocs_project_with_requirements / "mkdocs.yml")
        mock_parse_config = mocker.patch("devildex.grabbers.mkdocs_builder._parse_mkdocs_config", return_value={"site_name": "Test Project", "theme": "readthedocs"})
        mock_gather_packages = mocker.patch("devildex.grabbers.mkdocs_builder._gather_mkdocs_required_packages", return_value=["mkdocs"])
        ...
        mock_venv_manager = mocker.patch("devildex.grabbers.mkdocs_builder.IsolatedVenvManager")
        mock_venv_manager.return_value.__enter__.return_value = mock_venv_instance
        mock_install_deps = mocker.patch("devildex.grabbers.mkdocs_builder.install_project_and_dependencies_in_venv", return_value=True)

        # Mock execute_command for mkdocs build
        mock_execute_command = mocker.patch("devildex.grabbers.mkdocs_builder.execute_command", return_value=("stdout", "stderr", 0))
        ...
        result = builder.generate_docset(mkdocs_project_with_requirements, output_path, mock_build_context)

        assert result is True
        assert (output_path / mock_build_context.project_slug / mock_build_context.version_identifier).exists()
        ...
        mock_install_deps.assert_called_once_with(
            ...
            doc_requirements_path=_find_mkdocs_doc_requirements_file(
                mkdocs_project_with_requirements, mock_build_context.project_root_for_install, mock_build_context.project_slug
            ),
        ...
        # We need to extract the temporary file path from the mock_execute_command arguments.
        config_file_arg_index = args[0].index("--config-file") + 1
        temp_config_file_path_in_command = args[0][config_file_arg_index]
        assert "tmp" in temp_config_file_path_in_command and ".yml" in temp_config_file_path_in_command
        assert str((output_path / mock_build_context.project_slug / mock_build_context.version_identifier).resolve()) in args[0]
        assert kwargs["cwd"] == mkdocs_project_with_requirements

    def test_generate_docset_build_failure(self, mocker: MockerFixture, mkdocs_project_setup: Path, mock_build_context: BuildContext, tmp_path: Path) -> None:
        """Test generate_docset returns False when mkdocs build command fails."""
        builder = MkDocsBuilder()

        # Mock internal helper methods
        mocker.patch("devildex.grabbers.mkdocs_builder._find_mkdocs_config_file", return_value=mkdocs_project_setup / "mkdocs.yml")
        mocker.patch("devildex.grabbers.mkdocs_builder._parse_mkdocs_config", return_value={"site_name": "Test Project", "theme": "readthedocs"})
        mocker.patch("devildex.grabbers.mkdocs_builder._gather_mkdocs_required_packages", return_value=["mkdocs"])
        ...
        mock_venv_manager = mocker.patch("devildex.grabbers.mkdocs_builder.IsolatedVenvManager")
        mock_venv_manager.return_value.__enter__.return_value = mock_venv_instance
        mocker.patch("devildex.grabbers.mkdocs_builder.install_project_and_dependencies_in_venv", return_value=True)

        # Mock execute_command to simulate failure
        mocker.patch("devildex.grabbers.mkdocs_builder.execute_command", return_value=("stdout", "stderr", 1))

        output_path = tmp_path / "build_output"
        mock_build_context.base_output_dir = output_path

        result = builder.generate_docset(mkdocs_project_setup, output_path, mock_build_context)

        assert result is False

    def test_generate_docset_no_mkdocs_yml(self, tmp_path: Path, mock_build_context: BuildContext) -> None:
        """Test generate_docset returns False if mkdocs.yml is not found."""
        builder = MkDocsBuilder()
        source_path = tmp_path / "empty_project"
        source_path.mkdir()

        result = builder.generate_docset(source_path, tmp_path / "output", mock_build_context)

        assert result is False

    def test_find_mkdocs_doc_requirements_file_found(self, mkdocs_project_with_requirements: Path) -> None:
    ...
    def test_find_mkdocs_doc_requirements_file_not_found(self, mkdocs_project_setup: Path, caplog: pytest.LogCaptureFixture) -> None:
        """Test _find_mkdocs_doc_requirements_file returns None if no file is found."""
        builder = MkDocsBuilder()
        with caplog.at_level(logging.INFO):
            result = _find_mkdocs_doc_requirements_file(
                mkdocs_project_setup,
                mkdocs_project_setup,
                "test_project"
            )
    ...
    def test_gather_mkdocs_required_packages_material_theme(self, mocker: MockerFixture) -> None:
    ...
    def test_gather_mkdocs_required_packages_plugins(self, mocker: MockerFixture) -> None:
        ...
        config = {
            ...
            "plugins": [
                "search",
                {"mkdocstrings": {"handlers": {"python": {"options": {"show_root_heading": True}}}}}
            ],
            "markdown_extensions": [
                "pymdownx.highlight",
                "admonition"
    ...
    def test_parse_mkdocs_config_invalid_yaml(self, tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    def test_parse_mkdocs_config_file_not_found(self, tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
