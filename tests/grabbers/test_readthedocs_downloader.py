...
@pytest.fixture
def mock_build_context(mocker: MockerFixture, tmp_path: Path) -> BuildContext:
    ...
    context = BuildContext(
        ...
        project_root_for_install=install_dir,
...
def test_generate_docset_empty_project_slug(
    ...
    result = rtd_downloader.generate_docset(
        source_path=mock_build_context.source_root,
        output_path=mock_build_context.base_output_dir,
...
def test_can_handle_returns_true_for_readthedocs(
    ...
    assert (
        rtd_downloader.can_handle(mock_build_context.source_root, mock_build_context)
        is True
    )
