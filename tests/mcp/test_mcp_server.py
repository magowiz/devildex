...
def mcp_server_with_populated_db():
    ...
    with database.get_session() as session:
        ...
        docset_flask = Docset(
            package_name="flask", package_version="2.0.0", status="available", package_info=pkg_info_flask
        )
        ...
        docset_django = Docset(
            package_name="django", package_version="3.2.0", status="available", package_info=pkg_info_django
        )
        session.add_all([pkg_info_django, docset_django])
        session.commit()

        # Add PackageInfo and Docset for "numpy"
        pkg_info_numpy = PackageInfo(package_name="numpy", summary="Numerical computing.")
        docset_numpy = Docset(
            package_name="numpy", package_version="1.20.0", status="available", package_info=pkg_info_numpy
        )
        ...
        docset_pandas = Docset(
            package_name="pandas", package_version="1.3.0", status="available", package_info=pkg_info_pandas
        )
    ...
    try:
        ...
        server_command = [
            "poetry", "run", "python", "src/devildex/mcp_server/server.py"
        ]
...
async def test_get_docsets_list_all_projects(mcp_server_with_populated_db: DevilDexCore) -> None:
    ...
    async with client:
        try:
            ...
            assert sorted(docsets_list.data) == sorted(expected_names) # Check exact content
...
async def test_get_docsets_list_by_project(mcp_server_with_populated_db: DevilDexCore) -> None:
    ...
    async with client:
        try:
            ...
            docsets_list = await client.call_tool("get_docsets_list", {"project": "TestProject"}, timeout=5)
...
async def test_get_docsets_list_invalid_params(mcp_server_with_populated_db: DevilDexCore) -> None:
    ...
    async with client:
        try:
            ...
            expected_error = {"error": "invalid parameters: project or all_projects must be provided"}
