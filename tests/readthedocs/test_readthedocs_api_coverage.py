...
def test_get_unique_branches_to_attempt() -> None:
    ...
    assert _get_unique_branches_to_attempt("main,master") == ["main,master", "master", "main"]
...
def test_attempt_single_branch_clone_failed_retryable(mocker, tmp_path: Path, caplog) -> None:
    """Verify failed clone attempt (retryable)."""
    mock_subprocess_run = mocker.patch("subprocess.run", return_value=MagicMock(returncode=1, stdout="", stderr="clone failed"))
    clone_dir = tmp_path / "repo"
...
def test_attempt_single_branch_clone_failed_critical_prepare_dir(mocker, tmp_path: Path, caplog) -> None:
...
def test_attempt_single_branch_clone_failed_critical_vcs_not_found_exec(mocker, tmp_path: Path, caplog) -> None:
    ...
    assert "Error: VCS command (/usr/bin/git) not found during subprocess.run." in caplog.text
...
def test_run_clone_success(mocker, tmp_path: Path) -> None:
    ...
    mocker.patch("devildex.readthedocs.readthedocs_src._get_vcs_executable", return_value="/usr/bin/git")
...
def test_find_doc_source_in_clone_not_found(tmp_path: Path, caplog) -> None:
    ...
    assert result is None
    assert "No documentation source directory with conf.py found in the clone." in caplog.text
...
def test_build_sphinx_docs_success(mocker, tmp_path: Path) -> None:
    ...
    mock_execute_command = mocker.patch("devildex.readthedocs.readthedocs_src.execute_command", return_value=("", "", 0))
    
    # Configure the mock IsolatedVenvManager
    mock_venv_manager_cls = mocker.patch("devildex.readthedocs.readthedocs_src.IsolatedVenvManager")
    mock_venv_instance = mock_venv_manager_cls.return_value.__enter__.return_value # Get the instance returned by 'with' statement
    mock_venv_instance.python_executable = "/usr/bin/python" # Set it to a string

    mocker.patch("devildex.readthedocs.readthedocs_src.install_project_and_dependencies_in_venv", return_value=True)
