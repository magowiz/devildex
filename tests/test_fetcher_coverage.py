...
BASE_SAVE_PATH = pathlib.Path("/tmp/devildex_test_output")
DUMMY_PACKAGE_INFO = {"name": "test_package", "version": "1.0.0"}

@pytest.fixture(autouse=True)
def setup_and_teardown_test_env():
    # Ensure base_save_path exists before tests
    ...
    if BASE_SAVE_PATH.exists():
        ...
        except (FileNotFoundError, OSError): # Catch OSError as well
            # Ignore FileNotFoundError and OSError, as another process might have already removed it or it's not empty
            pass
...
class TestPackageSourceFetcherCoverage:
    ...
    def test_is_path_safe_raises_exception(self, mock_resolve):
        ...
        assert not PackageSourceFetcher._is_path_safe(base_path, target_path)
    ...
    def test_extract_zip_safely_unsafe_member_name(self):
        ...
        zip_path = BASE_SAVE_PATH / "unsafe.zip"
        ...
        with patch('src.devildex.fetcher.PackageSourceFetcher._is_member_name_safe', return_value=False):
            assert not PackageSourceFetcher._extract_zip_safely(zip_path, BASE_SAVE_PATH / "extracted")

    def test_extract_zip_safely_bad_zip_file(self):
        # Create a dummy corrupted zip file
        zip_path = BASE_SAVE_PATH / "corrupted.zip"
    ...
    def test_extract_zip_safely_os_error_during_extraction(self, mock_zip_file):
        ...
        mock_zip_instance.infolist.return_value = [MagicMock(filename="file.txt", is_dir=lambda: False)]
        mock_zip_instance.extract.side_effect = OSError("Disk full")

        zip_path = BASE_SAVE_PATH / "dummy.zip"
    ...
    def test_extract_tar_safely_unsafe_member_name(self, mock_tar_open):
        ...
        mock_tar_instance.getmembers.return_value = [MagicMock(name="../evil.txt", isfile=lambda: True, isdir=lambda: False)]

        # Mock _is_member_name_safe to return False for the unsafe name
        with patch('src.devildex.fetcher.PackageSourceFetcher._is_member_name_safe', return_value=False):
            tar_path = BASE_SAVE_PATH / "dummy.tar.gz"
            tar_path.touch() # Create a dummy file for the path check
            assert not PackageSourceFetcher._extract_tar_safely(tar_path, BASE_SAVE_PATH / "extracted")
    ...
    def test_extract_tar_safely_bad_tar_file(self, mock_tar_open):
        ...
        tar_path = BASE_SAVE_PATH / "corrupted.tar.gz"
    ...
    def test_extract_tar_safely_tar_error_during_extraction(self, mock_tar_open):
        mock_tar_instance = MagicMock()
        mock_tar_open.return_value.__enter__.return_value = mock_tar_instance
        mock_tar_instance.getmembers.return_value = [MagicMock(name="file.txt", isfile=lambda: True, isdir=lambda: False)]
    ...
    def test_determine_content_source_dir_empty(self):
        empty_dir = BASE_SAVE_PATH / "empty_dir"
        empty_dir.mkdir()
    ...
    def test_determine_content_source_dir_multiple_items(self):
        multi_item_dir = BASE_SAVE_PATH / "multi_item_dir"
        multi_item_dir.mkdir(parents=True)
        (multi_item_dir / "file1.txt").touch()
        (multi_item_dir / "dir1").mkdir()
        assert PackageSourceFetcher._determine_content_source_dir(multi_item_dir) == multi_item_dir

    def test_determine_content_source_dir_single_file(self):
        single_file_dir = BASE_SAVE_PATH / "single_file_dir"
        single_file_dir.mkdir()
        (single_file_dir / "file1.txt").touch()
        assert PackageSourceFetcher._determine_content_source_dir(single_file_dir) == single_file_dir
    ...
    def test_move_extracted_content_os_error(self, mock_shutil_move):
        ...
        source_dir = BASE_SAVE_PATH / "source"
        source_dir.mkdir()
        (source_dir / "file.txt").touch()
        destination_dir = BASE_SAVE_PATH / "destination"
        destination_dir.mkdir()
        assert not PackageSourceFetcher._move_extracted_content(source_dir, destination_dir)
    ...
    def test_download_and_extract_archive_temp_base_dir_exists(self, mock_move, mock_ensure_target_dir, mock_cleanup_target_dir, mock_determine_content_source_dir, mock_extract_archive, mock_download_file):
        temp_base_dir = BASE_SAVE_PATH / "temp_base_dir_exists"
        ...
        mock_determine_content_source_dir.return_value = BASE_SAVE_PATH / "content_source"
        mock_ensure_target_dir.return_value = True
        mock_move.return_value = True

        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert fetcher._download_and_extract_archive("http://example.com/archive.zip", temp_base_dir)
        # Check if shutil.rmtree was called on temp_base_dir
        assert not temp_base_dir.exists() # shutil.rmtree should have removed it

    @patch('src.devildex.fetcher.PackageSourceFetcher._extract_archive', return_value=False)
    def test_download_and_extract_archive_extract_fails(self, mock_extract_archive):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._download_and_extract_archive("http://example.com/archive.zip", BASE_SAVE_PATH / "temp_extract_fails")

    @patch('src.devildex.fetcher.PackageSourceFetcher._extract_archive', return_value=True)
    @patch('src.devildex.fetcher.PackageSourceFetcher._determine_content_source_dir', return_value=None)
    def test_download_and_extract_archive_determine_content_fails(self, mock_determine_content_source_dir, mock_extract_archive):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._download_and_extract_archive("http://example.com/archive.zip", BASE_SAVE_PATH / "temp_determine_fails")

    @patch('src.devildex.fetcher.PackageSourceFetcher._extract_archive', return_value=True)
    @patch('src.devildex.fetcher.PackageSourceFetcher._determine_content_source_dir', return_value=BASE_SAVE_PATH / "content_source")
    @patch('src.devildex.fetcher.PackageSourceFetcher._ensure_target_dir_exists', return_value=False)
    def test_download_and_extract_archive_ensure_target_fails(self, mock_ensure_target_dir, mock_determine_content_source_dir, mock_extract_archive):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._download_and_extract_archive("http://example.com/archive.zip", BASE_SAVE_PATH / "temp_ensure_target_fails")
    ...
    def test_download_and_extract_archive_requests_exception(self, mock_requests_get):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._download_and_extract_archive("http://example.com/archive.zip", BASE_SAVE_PATH / "temp_requests_exception")
    ...
    def test_run_git_command_non_git_first_element(self, mock_subprocess_run, mock_which):
        mock_process = MagicMock(stdout="output", stderr="", returncode=0)
        mock_subprocess_run.return_value = mock_process
        result = PackageSourceFetcher._run_git_command(["clone", "url"], check_errors=False)
        mock_subprocess_run.assert_called_once_with(
            ["/usr/bin/git", "clone", "url"],
            capture_output=True, text=True, check=False, cwd=None, encoding="utf-8", errors="replace"
        )
    ...
    def test_run_git_command_stdout_logged(self, mock_logger, mock_subprocess_run, mock_which):
    ...
    @patch('shutil.which', return_value="/usr/bin/git")
    @patch('subprocess.run', side_effect=subprocess.CalledProcessError(1, "git clone", stderr="error output"))
    @patch('src.devildex.fetcher.logger')
    def test_run_git_command_called_process_error_caught(self, mock_logger, mock_subprocess_run, mock_which):
    ...
    @patch('subprocess.run')
    @patch('src.devildex.fetcher.logger')
    def test_run_git_command_stderr_warning_logged(self, mock_logger, mock_subprocess_run, mock_which):
        mock_process = MagicMock(stdout="", stderr="some error", returncode=1)
        mock_subprocess_run.return_value = mock_process
        PackageSourceFetcher._run_git_command(["git", "pull"], check_errors=False) # check_errors=False to allow stderr without raising
        mock_logger.warning.assert_any_call("Git stderr:\nsome error")

    @patch('shutil.which', return_value="/usr/bin/git")
    @patch('subprocess.run')
    @patch('src.devildex.fetcher.logger')
    def test_run_git_command_stderr_info_logged(self, mock_logger, mock_subprocess_run, mock_which):
        mock_process = MagicMock(stdout="", stderr="some info", returncode=0)
        mock_subprocess_run.return_value = mock_process
        PackageSourceFetcher._run_git_command(["git", "status"])
    ...
    def test_cleanup_git_dir_from_path_os_error(self, mock_rmtree):
        git_dir = BASE_SAVE_PATH / ".git"
        git_dir.mkdir()
        assert not PackageSourceFetcher._cleanup_git_dir_from_path(BASE_SAVE_PATH)

    def test_cleanup_git_dir_from_path_git_is_file(self):
        git_file = BASE_SAVE_PATH / ".git"
    ...
    def test_try_fetch_tag_github_archive_non_github_url(self):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._try_fetch_tag_github_archive("http://example.com/repo.git", ["v1.0.0"])

    @patch('src.devildex.fetcher.PackageSourceFetcher._download_and_extract_archive', return_value=False)
    def test_try_fetch_tag_github_archive_all_attempts_fail(self, mock_download_and_extract_archive):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
    ...
    @patch('src.devildex.fetcher.PackageSourceFetcher._ensure_target_dir_exists', return_value=False)
    def test_try_fetch_tag_shallow_clone_ensure_target_fails(self, mock_ensure_target_dir_exists):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        repo_url = "https://github.com/user/repo.git"
    ...
    def test_copy_cloned_content_skips_git_dir(self):
        source_dir = BASE_SAVE_PATH / "source_with_git"
        source_dir.mkdir()
        (source_dir / ".git").mkdir()
        (source_dir / "file.txt").touch()
        destination_dir = BASE_SAVE_PATH / "destination_no_git"
        destination_dir.mkdir()
    ...
    def test_copy_cloned_content_copytree_os_error(self, mock_copytree):
        source_dir = BASE_SAVE_PATH / "source_dir_error"
        source_dir.mkdir()
        (source_dir / "subdir").mkdir()
        destination_dir = BASE_SAVE_PATH / "destination_dir_error"
        destination_dir.mkdir()
        assert not PackageSourceFetcher._copy_cloned_content(source_dir, destination_dir)

    @patch('shutil.copy2', side_effect=OSError("Permission denied"))
    def test_copy_cloned_content_copy2_os_error(self, mock_copy2):
        source_dir = BASE_SAVE_PATH / "source_file_error"
        source_dir.mkdir()
        (source_dir / "file.txt").touch()
        destination_dir = BASE_SAVE_PATH / "destination_file_error"
        destination_dir.mkdir()
        assert not PackageSourceFetcher._copy_cloned_content(source_dir, destination_dir)
    ...
    @patch('src.devildex.fetcher.logger')
    def test_try_fetch_tag_variations_checkout_success_copy_success(self, mock_logger, mock_copy_cloned_content, mock_ensure_target_dir_exists, mock_run_git_command):
        ...
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        temp_clone_dir = BASE_SAVE_PATH / "temp_clone"
        temp_clone_dir.mkdir()
        assert fetcher._try_fetch_tag_variations(["v1.0.0"], temp_clone_dir)
        mock_logger.info.assert_called_with(f"Checkout of tag 'v1.0.0' successful in {temp_clone_dir}.")

    @patch('src.devildex.fetcher.PackageSourceFetcher._run_git_command')
    @patch('src.devildex.fetcher.PackageSourceFetcher._ensure_target_dir_exists', return_value=False)
    def test_try_fetch_tag_variations_ensure_target_fails(self, mock_ensure_target_dir_exists, mock_run_git_command):
        mock_run_git_command.return_value = MagicMock(returncode=0)
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        temp_clone_dir = BASE_SAVE_PATH / "temp_clone"
        temp_clone_dir.mkdir()
    ...
    @patch('src.devildex.fetcher.PackageSourceFetcher._run_git_command')
    @patch('src.devildex.fetcher.PackageSourceFetcher._try_fetch_tag_variations', return_value=True)
    def test_try_fetch_tag_full_clone_checkout_success(self, mock_try_fetch_tag_variations, mock_run_git_command):
        mock_run_git_command.return_value = MagicMock(returncode=0)
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert fetcher._try_fetch_tag_full_clone_checkout("http://example.com/repo.git", ["v1.0.0"])

    @patch('src.devildex.fetcher.PackageSourceFetcher._run_git_command', return_value=None)
    def test_try_fetch_tag_full_clone_checkout_clone_fails(self, mock_run_git_command):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._try_fetch_tag_full_clone_checkout("http://example.com/repo.git", ["v1.0.0"])
    ...
    def test_fetch_from_vcs_tag_version_starts_with_v(self):
        ...
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, package_info)
        # Mock the actual fetch methods to avoid network calls
        with patch('src.devildex.fetcher.PackageSourceFetcher._try_fetch_tag_github_archive', return_value=False), \
             patch('src.devildex.fetcher.PackageSourceFetcher._try_fetch_tag_shallow_clone', return_value=False), \
             patch('src.devildex.fetcher.PackageSourceFetcher._try_fetch_tag_full_clone_checkout', return_value=False):
    ...
    @patch('src.devildex.fetcher.PackageSourceFetcher._ensure_target_dir_exists', return_value=False)
    @patch('src.devildex.fetcher.logger')
    def test_fetch_from_vcs_main_ensure_target_fails(self, mock_logger, mock_ensure_target_dir_exists):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        assert not fetcher._fetch_from_vcs_main("http://example.com/repo.git")
        mock_logger.error.assert_called_with("Unable to prepare target directory for the VCS main branch clone.")
    ...
    def test_fetch_target_dir_exists_with_content(self):
        ...
        with patch('src.devildex.fetcher.PackageSourceFetcher._cleanup_git_dir_from_path') as mock_cleanup:
    ...
    @patch('src.devildex.fetcher.PackageSourceFetcher._fetch_from_pypi', return_value=False)
    @patch('src.devildex.fetcher.PackageSourceFetcher._get_vcs_url', return_value=None)
    @patch('src.devildex.fetcher.PackageSourceFetcher._cleanup_target_dir_content')
    def test_fetch_all_methods_fail_cleanup_called(self, mock_cleanup_target_dir_content, mock_get_vcs_url, mock_fetch_from_pypi):
        fetcher = PackageSourceFetcher(BASE_SAVE_PATH, DUMMY_PACKAGE_INFO)
        success, is_master, path_str = fetcher.fetch()
        assert not success
        assert not is_master
    ...
    def test_pprint_function_logs_json(self, mock_logger):
        ...
        assert "{\n    \"key\": \"value\",\n    \"num\": 123\n}" in logged_message or "{\n    \"num\": 123,\n    \"key\": \"value\"\n}" in logged_message # Order might vary
