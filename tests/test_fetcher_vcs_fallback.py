...
def fetcher(tmp_path: Path) -> PackageSourceFetcher:
    """Provides a PackageSourceFetcher instance for testing VCS fallbacks."""
    package_info = {
        "name": "vcs-test-pkg",
        "version": "1.2.3",
        "project_urls": {},
    }
    return PackageSourceFetcher(base_save_path=str(tmp_path), package_info_dict=package_info)
...
def test_fetch_github_archive_success(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    mock_download_extract = mocker.patch.object(fetcher, "_download_and_extract_archive", return_value=True)
    ...
    # It should try the first URL format for the first tag, succeed, and stop.
...
def test_fetch_github_archive_falls_back_on_urls(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    archive_urls_to_try = [
        ...
        f"https://github.com/{repo_path_segment}/archive/{tag_to_test}.zip",  # This one will "succeed"
    ]
    # Simulate failure on the first 3 URLs, then success on the 4th
    ...
    expected_calls = [call(url, mocker.ANY, from_vcs=True) for url in archive_urls_to_try]
...
def test_fetch_github_archive_all_fail(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    repo_url = "https://github.com/user/repo"
    tag_variations = ["1.2.3", "v1.2.3"]
    # Mock the download method to always fail
    mock_download_extract = mocker.patch.object(fetcher, "_download_and_extract_archive", return_value=False)
    ...
    # It should have tried all 4 URL formats for EACH of the 2 tags = 8 attempts
...
def test_fetch_github_archive_not_a_github_url(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    mock_download_extract = mocker.patch.object(fetcher, "_download_and_extract_archive")
    ...
    # Assert
    assert result is False
    # The download method should never have been called
...
def test_shallow_clone_success(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    mock_run_git.assert_called_once_with([
        ...
        repo_url, str(fetcher.download_target_path)
    ])
...
def test_full_clone_checkout_success(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    # Mock the git commands: first for clone, second for checkout
...
def test_full_clone_fails(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    # Assert
    assert result is False
    # rmtree should not be called if cloning never even created the dir
...
def test_full_clone_checkout_fails(fetcher: PackageSourceFetcher, mocker: MockerFixture):
    ...
    # Succeed on clone, fail on all checkouts
    mock_git_clone = mocker.Mock(returncode=0)
