"""Tests for the ReadTheDocs API client."""

import pytest

from devildex.readthedocs.readthedocs_api import (
    _choose_best_version as choose_best_version,
)
...
def test_choose_best_version_finds_first_preference(
    ...
    result = choose_best_version(sample_versions, preferred)
...
def test_choose_best_version_handles_no_active_built_versions() -> None:
    ...
    preferred = ["stable", "latest"]
    result = choose_best_version(versions, preferred)
...
def test_choose_best_version_handles_empty_version_list() -> None:
    ...
    preferred = ["stable", "latest"]
    result = choose_best_version([], preferred)
    assert result is None
