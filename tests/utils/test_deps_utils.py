...
def test_filter_requirements_lines_requirements_file_none(caplog) -> None:
...
def test_process_requirements_obj_invalid_lines(caplog) -> None:
    ...
    mock_req_file.invalid_lines = [
        ...
        MagicMock(line=""), # Empty line
        MagicMock(line=None) # None line
    ]
    ...
    assert "in ' /fake/reqs.txt'." in record.message # Corrected assertion for space and period


def test_process_requirements_obj_explicit_removal(caplog) -> None:
    ...
    with caplog.at_level(logging.INFO):
        result = _process_requirements_obj(mock_req_file, Path("/fake/reqs.txt"), lines_to_remove)

    assert result == ["valid-dep"]
    assert "Explicit removal of the line '-e .' from '/fake/reqs.txt'" in caplog.text # Corrected assertion


@patch("devildex.utils.deps_utils.logger.error")
@patch("pathlib.Path.exists", return_value=False)
def test_filter_requirements_lines_file_not_found(mock_exists, mock_logger_error) -> None:
    """Verify filter_requirements_lines handles file not found."""
    result = filter_requirements_lines("/fake/path/non_existent.txt")
    assert result is None
    mock_logger_error.assert_called_once_with("requirements file '%s' not found.", Path("/fake/path/non_existent.txt")) # Corrected assertion


@patch("devildex.utils.deps_utils.logger.debug")
@patch("devildex.utils.deps_utils.RequirementsFile.from_file")
@patch("pathlib.Path.exists", return_value=True)
def test_filter_requirements_lines_success(mock_exists, mock_from_file, mock_logger_debug) -> None:
    """Verify filter_requirements_lines successfully filters lines."""
    mock_req_file = MagicMock(spec=RequirementsFile)
    mock_req_file.requirements = [
        MagicMock(line="requests"),
        MagicMock(line="-e ."), # This should be filtered by _process_requirements_obj
    ]
    mock_req_file.invalid_lines = []
    ...
    mock_logger_debug.assert_any_call("Numero di rows valid extract da '%s': %s", Path("/fake/path/reqs.txt"), 1)


@patch("devildex.utils.deps_utils.logger.exception")
@patch("devildex.utils.deps_utils.RequirementsFile.from_file", side_effect=OSError("Read error"))
@patch("pathlib.Path.exists", return_value=True)
def test_filter_requirements_lines_os_error(mock_exists, mock_from_file, mock_logger_exception) -> None:
    """Verify filter_requirements_lines handles OSError during parsing."""
    result = filter_requirements_lines("/fake/path/reqs.txt")
    assert result is None
    mock_logger_exception.assert_called_once()
    assert "Decode or I/O error during parsing file '%s'" in mock_logger_exception.call_args[0][0]
    assert Path("/fake/path/reqs.txt") in mock_logger_exception.call_args[0]


@patch("devildex.utils.deps_utils.logger.exception")
@patch("devildex.utils.deps_utils.RequirementsFile.from_file", side_effect=UnicodeDecodeError("utf-8", b"\x80", 0, 1, "invalid byte"))
@patch("pathlib.Path.exists", return_value=True)
def test_filter_requirements_lines_unicode_error(mock_exists, mock_from_file, mock_logger_exception) -> None:
